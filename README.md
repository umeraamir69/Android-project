...
- **Firebase Auth** — email/password (create account + sign in)
- **Library sync** — Firestore mirror of courses / lectures / notes / transcript / chat / handout metadata
- **On-device mode** — SpeechRecognizer + scored extractive notes / Ask AI (Settings → Processing mode)
...
4. Firebase Console:
   ...
   - Enable **Storage** (handouts + shared files) — deploy rules from [`docs/firebase-storage.rules`](docs/firebase-storage.rules)
   - Allow authenticated writes to `users/{uid}/**` and `shared/{code}/**`
...
- Firebase Auth (email) + Firestore library sync (incl. chat / handouts)  
- Privacy path: on-device extractive processing without cloud STT/LLM  
